<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['demail'])
    {
      ...
      header("location:quizDashboard.php?q=1");
    }
  }
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'rmquiz') 
    {
      ...
      header("location:quizDashboard.php?q=5");
    }
  }
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addquiz') 
    {
      ...
      header("location:quizDashboard.php?q=4&step=2&eid=$id&n=$total");
    }
  }
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    {
      $sn++;
      header("location:quizWelcome.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
    }
    else if( $_SESSION['key']!='Chinmay')
    {
      ...
      $q=mysqli_query($conn,"SELECT title FROM quiz WHERE eid='$eid'" )or die('Error161');
      while($row=mysqli_fetch_array($q) )
      {
        $quizName=$row['title'];
      }
      ...
      if($rowcount == 0)
      {
        $q2=mysqli_query($conn,"INSERT INTO rank VALUES('$email','$s',NOW(),'$quizName')")or die('Error165');
      }
      else
      {
        while($row=mysqli_fetch_array($q) )
        ...
        $q=mysqli_query($conn,"UPDATE `rank` SET `score`=$sun ,time=NOW(),`testname`='$quizName' WHERE email= '$email'")or die('Error174');
      }
      header("location:quizWelcome.php?q=result&eid=$eid");
    }
    else
    {
      header("location:quizWelcome.php?q=result&eid=$eid");
  ...
  if(@$_GET['q']== 'quizre' && @$_GET['step']== 25 ) 
  {
    ...
    $q=mysqli_query($conn,"UPDATE `rank` SET `testname`='$quizName',`score`= $sun ,time=NOW() WHERE email= '$email'")or die('Error174');
    header("location:quizWelcome.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
  }
?>
